...
public class Salt2TreetaggerMapperTest {
	...
	protected SDocument createSDocument() {
		...
		{// creating the document structure
			...
			{// creating tokenization (token objects and relations between
				...
				sTextRel.setStart(0);
				// adding the end-position of the token in the primary text
				// (start-position of the token + length of the token)
				sTextRel.setEnd(2);
				// adding the textual relation between token and primary text to
				// document graph
				...
				{// creating the rest of the tokenization, this can also be done
					...
					// creating tokenization for the token 'this' and adding it
					// to the morphology layer
					sToken = SaltFactory.createSToken();
					sDocGraph.addNode(sToken);
					...
					// creating tokenization for the token 'example' and adding
					// it to the morphology layer
					sToken = SaltFactory.createSToken();
					sDocGraph.addNode(sToken);
					sTextRel = SaltFactory.createSTextualRelation();
					sTextRel.setSource(sToken);
					...
					// creating tokenization for the token 'more' and adding it
					// to the morphology layer
					sToken = SaltFactory.createSToken();
					sDocGraph.addNode(sToken);
					sTextRel = SaltFactory.createSTextualRelation();
					...
					// creating tokenization for the token 'complicated' and
					// adding it to the morphology layer
					sToken = SaltFactory.createSToken();
					sDocGraph.addNode(sToken);
					sTextRel = SaltFactory.createSTextualRelation();
					sTextRel.setSource(sToken);
					...
					// creating tokenization for the token 'than' and adding it
					// to the morphology layer
					sToken = SaltFactory.createSToken();
					sDocGraph.addNode(sToken);
					sTextRel = SaltFactory.createSTextualRelation();
					...
					// creating tokenization for the token 'it' and adding it to
					...
					// creating tokenization for the token 'appears' and adding
					// it to the morphology layer
					sToken = SaltFactory.createSToken();
					sDocGraph.addNode(sToken);
					sTextRel = SaltFactory.createSTextualRelation();
					...
					// creating tokenization for the token 'to' and adding it to
					// the morphology layer
					sToken = SaltFactory.createSToken();
					sDocGraph.addNode(sToken);
					sTextRel = SaltFactory.createSTextualRelation();
					...
					// creating tokenization for the token 'be' and adding it to
					// the morphology layer
					sToken = SaltFactory.createSToken();
					sDocGraph.addNode(sToken);
					sTextRel = SaltFactory.createSTextualRelation();
			...
			{// adding some annotations, part-of-speech and lemma (for part-of
				...
				{// adding part-of speech annotations
					...
					// a list of all part-of-speech annotations for the words Is
					// (VBZ), this (DT) ... be (VB)
				...
				{// creating annotations for information structure with the use
					...
					// creating a span node as placeholder for
			...
			{// creating an anaphoric relation with the use of pointing
				...
				// creating a span as placeholder, which contains the tokens for
				// "this" and "example"
	...
	/**
	 * Test method for
	 * {@link org.corpus_tools.peppermodules.treetagger.mapper.Salt2TreetaggerMapper#map(de.hu_berlin.german.korpling.saltnpepper.salt.saltCommon.sCorpusStructure.SDocument, de.hu_berlin.german.korpling.saltnpepper.misc.treetagger.Document)}
	 * .
	 */
	@Test
